...
\begin{example}
At the beginning of the semester, STAT 101, students have been asked to complete a general survey to obtain information about demographic and other variables. ata were collected from the Spring 2004 through Spring 2007 semesters from 2,068 students.
...
The way a variable is used is also important for determining whether or note the variable in question is quantitative or categorical. Circle whether each variable is categorial or quantative given its usage. 
